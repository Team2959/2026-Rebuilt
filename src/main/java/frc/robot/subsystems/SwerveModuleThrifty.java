...
public class SwerveModuleThrifty {
    ...
    // private static final double kWheelRadius = 1.90 * 0.0254; // 2" * 0.0254 m /
    // inch
    // private static final double kDrivePositionFactor = (2.0 * Math.PI *
    // kWheelRadius * kGearboxRatio);
    private static final double kGearboxRatio = 1.0 / 5.14; // One turn of the wheel is 5.14 turns of the motor, output
                                                            // gear 16T, pinion gear 14T
    ...
    private SwerveModuleThrifty(int driveMotor, int steerMotor, int steerAbsoluteEncoder, double steerOffset,
            String name) {
        m_name = name;
    ...
    private Rotation2d getThriftyEncoder() {
        ...
        // The signal pin is a 12-bit absolute position reference with the lower bound
        // being the ground pin and the
        // upper bound being the 5V pin. This 5V is relative to the 5V rail on the
        // Roborio, which is often not exactly
    ...
    public void checkRelativeEncoderToAbsoluteEncoder() {
        // we put that the difference should be less than 0.025 which is approximately
        // 0.5% margin of error
